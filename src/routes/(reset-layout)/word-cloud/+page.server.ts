...
export const actions = {
	add: async (event) => {
		...
		const validateInput = [word1, word2, word3]
			.map((word) => (word ? word.toString() : ''))
			.filter((word) => Boolean(word) && word && typeof word === 'string')
			.map((word) => word.toLowerCase().trim())
			.filter((word) => /^[a-z]+$/.test(word));

		console.log(validateInput);

		const words = Array.from(new Set(validateInput));
		...
		if (words.filter(Boolean).length < 3) {
			return {
				...
				success: false,
				messages: [
					"Can't submit an empty input field or duplicates, should only submit words (containing A-Za-z)."
				]
			};
		}
